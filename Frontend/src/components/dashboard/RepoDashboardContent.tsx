...
import type { RepoAnalysisData } from "@/utils/mock-data";
...
export default function RepoDashboardContent({
  ...
  const handleDownloadReport = () => {
    ...
    // Header
    doc.setFontSize(22);
    doc.setTextColor(38, 114, 255); // Sentinel Blue
    ...
    doc.setFontSize(14);
    doc.setTextColor(0);
    doc.text("Repository Identity", 14, 40);
    doc.setFontSize(10);
    doc.text(`Target: ${owner}/${name}`, 14, 48);
    doc.text(`Audit Branch: ${branch}`, 14, 54);
    doc.text(`Analysis Depth: ${commitDepth} commits`, 14, 60);

    // Risk Verdict
    doc.setFontSize(14);
    doc.text("Security Verdict", 14, 75);
    doc.setFontSize(12);
    ...
    doc.text(`Overall Risk Level: ${analysis.riskLevel.toUpperCase()}`, 14, 83);
    doc.text(`Risk Score: ${analysis.overallRiskScore}/100`, 14, 90);
    doc.setTextColor(0);
    ...
    // Contributors Matrix
    doc.setFontSize(14);
    doc.text("Contributor Trust Network", 14, (doc as any).lastAutoTable.finalY + 15);

    autoTable(doc, {
      startY: (doc as any).lastAutoTable.finalY + 20,
      head: [['Contributor', 'Trust Score', 'Intelligence Note']],
      body: rawData.contributors.map(c => [
        c.username,
        `${c.trustScore}%`,
        c.trustScore < 30 ? '⚠ Needs Review' : 'Verified Contributor'
      ]),
      theme: 'striped',
      ...
      <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 glass-panel p-8 rounded-[2.5rem] border-white/5 relative overflow-hidden">
        ...
        <div className="flex items-center gap-6 relative z-10">
          ...
          <div>
            ...
            <h1 className="text-4xl md:text-5xl font-bold font-headline tracking-tighter text-white uppercase">
              Repository <span className="text-secondary italic">Analysis</span>
            </h1>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-4 relative z-10">
          ...
          <Button
            ...
            className="h-12 px-8 rounded-xl bg-primary glow-primary font-headline text-[10px] tracking-widest uppercase shadow-2xl"
          >
