...
export default function ImmersiveScanner({ mode = 'repo', target, owner, onComplete }: ImmersiveScannerProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setProgress((prev) => {
        ...
        // Slower progress for deeper analysis feel (roughly 15-20 seconds)
  ...
  // Transition chain to create "Preparing" feel
  useEffect(() => {
    ...
    if (scanState === 'preparing') {
      const timer = setTimeout(() => {
        ...
        setProgress(100);
      }, 2000); // Deliberate delay while "Building Dashboard"
      return () => clearTimeout(timer);
    }
    if (scanState === 'finalizing') {
      const timer = setTimeout(() => {
        setScanState('completed');
      }, 1500); // Time to see "DIRECTING TO DASHBOARD"
      return () => clearTimeout(timer);
    }
  }, [scanState]);
      ...
      <div className="w-full max-w-7xl z-10 flex flex-col items-center">
        ...
        <header className="text-center mb-16 space-y-4">
          ...
          <motion.h1 
            ...
            className="text-4xl md:text-7xl font-bold font-headline tracking-tighter text-white uppercase"
          >
            {mode === 'repo' ? (
              <>AUDITING <span className="text-muted-foreground">{owner}/</span>{target}</>
            ) : (
              <>PROFILING <span className="text-primary">@{target}</span></>
            )}
          </motion.h1>
        </header>
